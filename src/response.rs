...
pub struct Response<Error> {
    ...
    data_fields: Vec<json::Value>,
}

impl<Error> Response<Error> {
    ...
    pub fn on<T: PathFragment>(&mut self, field: Option<T>, handler: impl Fn(T) -> json::Value) {
        if let Some(field) = field {
            self.data_fields.push(handler(field));
        }
    }

    pub fn resolve(&self) -> json::Value {
...
mod tests {
    ...
    fn basic_json_object() {
        ...
        response.on(Some(cats_request), |_req| json!({ "weight": 4 }));

        assert_eq!(response.resolve(), json!({ "data": { "weight": 4 } }));
    }
}
